...
def search(request):
    ...
    if (request.query_params.get('storeList')):
...
def start_launcher(request):
    ...
    if (request.query_params.get('storeList')):
        ...
        storeList = ['Ebay', "Amazon", "AliExpress"]
    keyword = request.query_params.get('keyword')
    page = request.query_params.get('page')

    res = launcher.launch(storeList, keyword)
    dataPage = pagenationManager.paginate(res,int(page),30)
    serializer = ProductSerializer(dataPage, many=True)
    return Response(serializer.data)
